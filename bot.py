...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("verify_"):
        ...
        if not_joined:
            # Still not joined all channels - show detailed list
            ...
            for ch in not_joined:
                if ch.get('invite_link'):
                    # Private channel
                    remaining_list.append(f"🔒 Private Channel {private_num}")
                    private_num += 1
                else:
                    # Public channel
                    remaining_list.append(f"📢 @{ch['username']}")
            
            remaining_text = "\n".join(remaining_list)
            
            # Create smart message based on count
            if len(not_joined) == 1:
                message = f"❌ **আরও ১টি চ্যানেল বাকি!**\n❌ **1 More Channel Needed!**\n\n📍 Join this channel:\n{remaining_text}\n\nThen click \"✅ আমি জয়েন করেছি\" again!"
            elif len(not_joined) == total_channels:
                message = f"❌ **সব চ্যানেল জয়েন করুন!**\n❌ **Join ALL Channels!**\n\n📍 Please join:\n{remaining_text}\n\nThen click \"✅ আমি জয়েন করেছি\"!"
            else:
                message = f"❌ **আরও {len(not_joined)}টি চ্যানেল বাকি!**\n❌ **{len(not_joined)} More Channels Needed!**\n\n📍 Join these channels:\n{remaining_text}\n\nThen click \"✅ আমি জয়েন করেছি\" again!"
